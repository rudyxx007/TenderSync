...
const API_BASE = ((import.meta.env.VITE_API_URL as string | undefined) || 'https://tendersync-ind-rudyxx007.loca.lt').replace(/\/+$/, '');
...
export async function apiFetch<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(response.status, err.detail || 'The request could not be completed.');
  }
...
export async function apiUpload<T>(path: string, formData: FormData): Promise<T> {
  ...
  if (!response.ok) {
    const err = await response.json().catch(() => ({ detail: response.statusText }));
    throw new ApiError(response.status, err.detail || 'The file upload failed.');
  }
  return response.json() as Promise<T>;
}
